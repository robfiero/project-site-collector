...
outbox_code="$(curl -sS -o "$OUTBOX_PREFLIGHT" -w "%{http_code}" "$BASE_URL/api/dev/outbox")"
...
curl -sS -o "$HEALTH_FILE" "$BASE_URL/api/health"
...
me_code="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -o "$ME_FILE" -w "%{http_code}" "$BASE_URL/api/me")"
echo "HTTP $me_code"
...
get_code="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -o "$GET_PREFS_FILE" -w "%{http_code}" "$BASE_URL/api/me/preferences")"
...
me_after_code="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -o "$ME_AFTER_LOGOUT" -w "%{http_code}" "$BASE_URL/api/me")"
...
outbox_code2="$(curl -sS -o "$OUTBOX_FILE" -w "%{http_code}" "$BASE_URL/api/dev/outbox")"
...
me_reset_code="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -o "$ME_AFTER_RESET" -w "%{http_code}" "$BASE_URL/api/me")"
